...
func DiffGoMod(baseRef, headRef string) (PRDiffReport, error) {
	...
	for modPath, newVer := range added {
		if oldVer, wasPresent := removed[modPath]; wasPresent {
			caps := scanModuleCaps(modPath, newVer)
			oldCaps := scanModuleCaps(modPath, oldVer)
...
func scanModuleCaps(modulePath, version string) capability.CapabilitySet {
	tmpDir := fetchModuleSource(modulePath, version)
	if tmpDir == "" {
	...
	for _, pkg := range g.Packages {
		if pkg.Module != nil && pkg.Module.Path == modulePath {
			combined.Merge(pkg.Capabilities)
		}
	}
	return combined
}

func fetchModuleSource(modulePath, version string) string {
	modSpec := modulePath
	if version != "" {
